...
(test all-fourteen-are-present
  (is (= 14 (length settings:+profiles+)))
  (dolist (name '("Default Amber" "Monochrome Green" "Deep Blue" "Commodore 64"
                  ...
                  "Boring" "E-Ink"))
    (is (settings:find-profile name) "~S is missing" name)))
...
(test default-amber-is-what-it-should-be
  ...
  (let ((p (settings:find-profile "Default Amber")))
    (is (string= "#ff8100" (settings:profile-font-color p)))
    (is (string= "#000000" (settings:profile-background-color p)))
    (is (~~ 0.6d0 (settings:profile-bloom p)))
    (is (~~ 0.3d0 (settings:profile-burn-in p)))
    (is (~~ 0.2d0 (settings:profile-chroma-color p)))
    (is (= 0 (settings:profile-rasterization p)))
    (is (~~ 0.2d0 (settings:profile-screen-curvature p)))))

(test every-profile-round-trips-through-json
  "A profile must survive a trip through cool-retro-term's own file format."
  (dolist (p settings:+profiles+)
    (let ((back (settings:profile-from-json (settings:profile-to-json p))))
      (is (string= (settings:profile-name p) (settings:profile-name back))
          "~A lost its name" (settings:profile-name p))
      (is (string= (settings:profile-font-color p) (settings:profile-font-color back)))
      (is (~~ (settings:profile-bloom p) (settings:profile-bloom back) 1d-4)
          "~A: bloom ~F became ~F" (settings:profile-name p)
          (settings:profile-bloom p) (settings:profile-bloom back))
      (is (eq (settings:profile-blinking-cursor p)
              (settings:profile-blinking-cursor back))))))
...
file usable."
  (let ((p (settings:profile-from-alist '(("bloom" . 0.9d0))
                                        :into (copy-structure
                                               (settings:find-profile "Default Amber")))))
    (is (~~ 0.9d0 (settings:profile-bloom p)) "the named key changed")
    (is (string= "#ff8100" (settings:profile-font-color p))
        "an unnamed key must be left alone")))

(test unknown-keys-are-ignored
  (finishes (settings:profile-from-alist '(("somethingFromTheFuture" . 1)))))

(test wrong-version-is-refused
  (signals error
    (settings:profile-from-json "{\"version\": 99, \"bloom\": 0.5}")))

;;; Derived values --------------------------------------------------------------

(test burn-in-is-a-reciprocal
  "A LARGER burnIn setting is a SLOWER fade, because the shader gets a rate."
  (let ((slow (settings:make-profile :burn-in 1.0d0))
        (fast (settings:make-profile :burn-in 0.0d0)))
    (is (~~ (/ 1d0 1.6d0) (settings:burn-in-fade-time slow)))
    (is (~~ (/ 1d0 0.16d0) (settings:burn-in-fade-time fast)))
    (is (< (settings:burn-in-fade-time slow) (settings:burn-in-fade-time fast))
        "more burn-in must mean a lower decay rate")))

(test screen-radius-is-pixels-not-a-fraction
  (is (~~ 4.0d0 (settings:screen-radius (settings:make-profile :screen-radius 0d0)))
      "even zero rounds by four pixels")
  (is (~~ 120.0d0 (settings:screen-radius (settings:make-profile :screen-radius 1d0))))
  (is (~~ 27.2d0 (settings:screen-radius (settings:make-profile :screen-radius 0.2d0)))))

(test margin-accounts-for-the-corner
  "A rounded corner eats into the usable rectangle; the margin compensates."
  (let* ((square (settings:make-profile :margin 0d0 :screen-radius 0d0))
         (round (settings:make-profile :margin 0d0 :screen-radius 1d0)))
    (is (< (settings:margin square) (settings:margin round))
        "a rounder screen needs a bigger margin, or text runs under the bezel")
    (is (~~ (+ 1.0d0 (* (- 1d0 (/ 1d0 (sqrt 2d0))) 120d0))
            (settings:margin round)))))

(test frame-enabled-follows-any-of-three
  (is-false (settings:frame-enabled-p
             (settings:make-profile :ambient-light 0d0 :frame-size 0d0
  ...
  (dolist (key '(:ambient-light :frame-size :screen-curvature))
    ...
    (is-true (settings:frame-enabled-p
              (apply #'settings:make-profile
...
clean; below that they alias, so they fade rather than shimmer."
  (is (~~ 0d0 (settings:rasterization-intensity 100d0 100d0 100d0 100d0))
      "1x oversampling: no scanlines")
  (is (~~ 0d0 (settings:rasterization-intensity 100d0 100d0 200d0 200d0))
      "2x: still none")
  (is (~~ 1d0 (settings:rasterization-intensity 100d0 100d0 400d0 400d0))
      "4x: full strength")
  (is (< 0d0 (settings:rasterization-intensity 100d0 100d0 300d0 300d0) 1d0)
      "3x: partway up the ramp"))

(test contrast-moves-the-colours-apart
  (let ((low (settings:make-profile :contrast 0d0 :font-color "#ffffff"
                                    :background-color "#000000"))
        (high (settings:make-profile :contrast 1d0 :font-color "#ffffff"
                                     :background-color "#000000")))
    (flet ((spread (p) (- (util:rgba-r (settings:derived-font-color p))
                          (util:rgba-r (settings:derived-background-color p)))))
      (is (> (spread high) (spread low))
          "raising contrast must separate the derived colours"))
    (is (> (util:rgba-r (settings:derived-font-color low)) 0.1d0)
        "even at zero contrast the mix is 0.7, not 0 -- washed out, not blank")))

(test normalized-window-scale-keeps-geometry-constant
  (is (~~ 1d0 (settings:normalized-window-scale 1024d0 1024d0)))
  (is (> (settings:normalized-window-scale 512d0 512d0)
         (settings:normalized-window-scale 2048d0 2048d0))
