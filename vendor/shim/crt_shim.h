 ...
 * one.  libvterm's API crosses that line in seven places.
 *
 * There used to be an eighth, the variadic ioctl, and it is gone: the window
 * size is set from Lisp now, in src/pty/ioctl.lisp, because both
 * implementations CAN express a variadic call even though CFFI cannot.  That is
 * the first piece of this file to be replaced rather than argued about, and it
 * was chosen first because it is the smallest complete example of the shape --
 * one call, two implementations, everything else shared.  Rather than drag in cffi-libffi -- which
